...
class DbtRunner(BaseDbtRunner):
    ...
    def __init__(
        ...
        self.dbt_version = version.parse(dbt_version_string)


    def _run_command_with_dbt_python_api(self, dbt_command):
        dbt_logs = []
        if self.dbt_version.major >= 1 and self.dbt_version.minor >= 5:
            logger.info(f"DBT VERSION IS OK!!!!  {self.dbt_version}")
        else:
            logger.error(f"DBT VERSION IS NOT OK!!!! {self.dbt_version}")

        def collect_dbt_command_logs(event: EventMsg):
            if event.info.name == 'JinjaLogInfo':
                dict_event = MessageToDict(event)
                dbt_logs.append(json.dumps(dict_event))

        # initialize
        dbt = dbtRunner(callbacks=[collect_dbt_command_logs])
        # run the command
        res: dbtRunnerResult = dbt.invoke(dbt_command)
        result = '\n'.join(dbt_logs)
        logger.info(f"DBT COMMAND RESULT: {res}")
        return res.success, '\n'.join(dbt_logs)
    ...
    def _run_command(
        ...
        return self._run_command_with_dbt_python_api(dbt_command[1:])
        # try:
        ...
        #     )
        #     t1 = pc()
        #     logger.info(f"TIME FOR FULL RUN COMMAND: {t1-t0}")
    ...
    def run_operation(
        ...
        logger.info(f"NOAAA 2 {success} {command_output}")
