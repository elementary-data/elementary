...
def convert_local_time_to_timezone(local_time: datetime, timezone: Optional[str] = None) -> datetime:
...
def convert_time_to_timezone(time: datetime, timezone: Optional[str] = None) -> datetime:
...
def format_milliseconds(duration: int) -> str:
    ...
    remaining_milliseconds = duration - (
        hours * MILLISECONDS_IN_HOUR + minutes * MILLISECONDS_IN_MIN + seconds * MILLISECONDS_IN_SEC
    )
...
def datetime_strftime(datetime: datetime, include_timezone: bool = False) -> str:
    return datetime.strftime(DATETIME_FORMAT if not include_timezone else DATETIME_WITH_TIMEZONE_FORMAT)
...
def get_formatted_timedelta(time_ago_in_s: float) -> str:
    ...
    if days > 0:
        duration_sentence = f"{pluralize_string(days, 'day', 'days')} {hours}h {minutes}m {seconds}s"
    else:
        if hours > 0:
            duration_sentence = f"{pluralize_string(hours, 'hour', 'hours')} {minutes}m {seconds}s"
        else:
            if minutes > 0:
                duration_sentence = f"{pluralize_string(minutes, 'minute', 'minutes')} {seconds}s"
            else:
