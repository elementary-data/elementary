...
class TestManagementFetcher(FetcherClient):
    ...
    def get_models(
        ...
        models_results = json.loads(run_operation_response[0]) if run_operation_response else []
    ...
    def get_sources(
        ...
        sources_results = json.loads(run_operation_response[0]) if run_operation_response else []
    ...
    def get_resources_columns(self) -> DefaultDict[str, List[ResourceColumnModel]]:
        run_operation_response = self.dbt_runner.run_operation(macro_name="elementary_cli.get_resources_columns")
        resources_columns_results = json.loads(run_operation_response[0]) if run_operation_response else {}
    ...
    def _format_resources(
        ...
        for resource in resources:
            owners = unpack_and_flatten_and_dedup_list_of_strings(resource["owners"]) if resource["owners"] else []
            formatted_resources.append(
                ResourceModel(
                    ...
                    owners=owners,
                    columns=columns[f'{resource["database"]}.{resource["schema"]}.{resource["name"]}'.lower()],
                )
            )
    ...
    def get_tags(self) -> TagsModel:
        run_operation_response = self.dbt_runner.run_operation(macro_name="elementary_cli.get_project_tags")
        tags_results = json.loads(run_operation_response[0]) if run_operation_response else []
        all_tags = []
    ...
    def get_tests(self) -> List[TestModel]:
        run_operation_response = self.dbt_runner.run_operation(macro_name="elementary_cli.get_tests")
        test_results = json.loads(run_operation_response[0]) if run_operation_response else []
        tests = []
        for test_result in test_results:
            meta = json.loads(test_result["meta"])
            owners = unpack_and_flatten_and_dedup_list_of_strings(meta.get("owner", "[]"))
            model_owners = unpack_and_flatten_and_dedup_list_of_strings(test_result["model_owners"])
    ...
    def get_project_owners(self) -> List[str]:
        run_operation_response = self.dbt_runner.run_operation(macro_name="elementary_cli.get_project_owners")
        owners_results = json.loads(run_operation_response[0]) if run_operation_response else []
        all_owners = []
    ...
    def get_project_subscribers(self) -> List[str]:
        run_operation_response = self.dbt_runner.run_operation(macro_name="elementary_cli.get_resources_meta")
        resources_meta_results = json.loads(run_operation_response[0]) if run_operation_response else []
        all_subscribers = []
