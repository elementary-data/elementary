...
class AlertsFetcher(FetcherClient):
    ...
    def query_pending_alerts(self, days_back: int, type: Optional[AlertTypes] = None) -> List[PendingAlertSchema]:
        ...
        return [PendingAlertSchema(**result) for result in json.loads(pending_alerts_results[0])]

    def query_last_alert_times(self, days_back: int, type: Optional[AlertTypes] = None) -> Dict[str, str]:
