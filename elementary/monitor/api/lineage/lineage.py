...
from elementary.monitor.api.lineage.schema import LineageNodeSchema, LineageSchema, NodeDependsOnNodesSchema
...
class LineageAPI(APIClient):
    ...
    def get_lineage(self, exclude_elementary_models: bool = False) -> LineageSchema:
        ...
        nodes_depends_on_nodes = self._get_nodes_depends_on_nodes(exclude_elementary_models)
        for node_depends_on_nodes in nodes_depends_on_nodes:
            lineage_graph.add_edges_from([(node_depends_on_nodes.unique_id, depends_on_node) for depends_on_node in
                                          node_depends_on_nodes.depends_on_nodes])
    ...
    def get_dags(self) -> List[LineageSchema]:
        ...
        for node_depends_on_nodes in nodes_depends_on_nodes:
            edges = [(node_depends_on_nodes.unique_id, depends_on_node) for depends_on_node in
                     node_depends_on_nodes.depends_on_nodes]
    ...
    def _get_nodes_depends_on_nodes(self, exclude_elementary_models: bool = False) -> List[NodeDependsOnNodesSchema]:
        nodes_depends_on_nodes = []
        nodes_depends_on_nodes_results = self.dbt_runner.run_operation(
            macro_name="get_nodes_depends_on_nodes", macro_args={'exclude_elementary': exclude_elementary_models})
            ...
            for node_depends_on_nodes_result in json.loads(nodes_depends_on_nodes_results[0]):
                nodes_depends_on_nodes.append(NodeDependsOnNodesSchema(
                    ...
                    depends_on_nodes=json.loads(
                        node_depends_on_nodes_result.get("depends_on_nodes")) if node_depends_on_nodes_result.get(
                        "depends_on_nodes") else None,
                    type=node_depends_on_nodes_result.get("type")
                ))
        return nodes_depends_on_nodes

    @staticmethod
    def _convert_depends_on_node_to_lineage_node(nodes_depends_on_nodes: List[NodeDependsOnNodesSchema]) -> List[
        LineageNodeSchema]:
