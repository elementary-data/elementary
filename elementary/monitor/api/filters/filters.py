...
class FiltersAPI(APIClient):
    def get_filters(
        ...
        test_results_filters = self._get_test_filters(test_results_totals, models, sources)
    ...
    def _get_test_filters(
        ...
        for model_unique_id, total in totals.items():
            ...
            if not total.failures and not total.warnings and not total.errors and not total.passed:
    ...
    def _get_model_runs_filters(
        ...
        successful_runs_filter = FilterSchema(name="success", display_name="Successful Runs")
