...
TEST_LINK_TEXT = "View test runs"
MODEL_LINK_TEXT = "View model runs"
...
def _get_run_history_report_link(
    ...
    if unique_id and report_url:
        ...
        report_link = ReportLinkData(
            url=url, text=TEST_LINK_TEXT if path == ReportPath.TEST else MODEL_LINK_TEXT
        )
...
def get_test_runs_link(
    ...
    return _get_run_history_report_link(
        report_url, ReportPath.TEST, elementary_unique_id
    )
...
def get_model_runs_link(
    ...
    return _get_run_history_report_link(report_url, ReportPath.MODEL, model_unique_id)
...
def get_model_test_runs_link(
    ...
    if model_unique_id and report_url:
        formatted_report_url = _get_formatted_report_url(report_url)
        url = f'{formatted_report_url}/report/{ReportPath.TEST.value}/?treeNode={{"id":"{model_unique_id}"}}'
        report_link = ReportLinkData(url=url, text=TEST_LINK_TEXT)
