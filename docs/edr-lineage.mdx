...
After you successfully [installed Elementary](), [configured a connection profile]() and provided the permissions required to access the [query history](), it's time to create your data lineage graph using the lineage CLI.
...
The lineage is created from the data warehouse [query history]() and access history, and the timeframe filters are used to define which queries to include in the graph.
