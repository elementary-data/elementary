...
In order to visualize the data from the [dbt-package](/contributions#guide-on-contributing-to-the-dbt-package) tables, you can use the [CLI](/cli-install) to generate the Elementary report.
...
If you're receiving `command not found: edr` please check our [troubleshooting guide](/troubleshooting).
...
- If you want to change the schema elementary uses, please see [install & configuration](/quickstart-data-monitoring#install-the-dbt-package)
...
Before you can start using Elementary CLI, execute `dbt run` with the [Elementary package](/dbt-package) models, which are **required for Elementary CLI features to work.**
