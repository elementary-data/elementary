...
## 'edr lineage' advanced options

- **Profile name:**
  The relevant profile for use out of the profiles defined in your `profiles.yml` file. Default is `elementary`
  - This option must be configured together with profile directory path.

```shell
-p, --profile-name <TEXT>
```

## 'edr monitor' advanced options

When running the CLI, the following options may be used:
