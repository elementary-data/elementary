...
- **Control of schedule and cost** - You have control of configuration and schedualing, tests can be executed when data is actually loaded and validation is needed.
- **Prevent bad data from propogating** - As tests are in pipeline, you can leverage `dbt build` and fail the pipeline on critical test failures.
