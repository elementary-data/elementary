...
1. In your Databrick console, go to the admin settings by by clicking your username in the to right corner -> Admin settings (add photo)
...
```
Make sure to replace the `<elementary_schema>` and `<serivce_principal_id>` placeholders with the correct values
