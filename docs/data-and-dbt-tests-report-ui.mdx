...
Elementary has a UI for visualization and exploration of data from the [dbt-package](/contributions#guide-on-contributing-to-the-dbt-package) tables, which includes dbt test results, Elementary anomaly detection results, dbt artifacts, etc.

After installing and configuring the [CLI](/cli-install) and [dbt package](/quickstart-data-monitoring#install-the-dbt-package), execute the command:
...
The `Test results` tab shows the latest result for each dbt or Elementary test that was executed in the last 7 days (Can be changed by passing `--days-back` param to the [CLI](/cli-advanced)). 
...
<img src="img/data-observability-ui-test-results-screen.gif" alt="Data observability UI Test Result Screen" />

## Test returns

The `Test runs` tab shows the last 30 runs (can be changed by passing `--executions-limit` param to the [CLI](/cli-advanced)) for each dbt or Elementary test that was executed in the last 7 days (can be changed by passing `--days-back` param to the [CLI](/cli-advanced)).
...
<img src="img/Test-Runs-video-demo.gif" alt="Test runs demo" />
...
Refer to [this guide](/slack-alerts) for detailed instructions. 