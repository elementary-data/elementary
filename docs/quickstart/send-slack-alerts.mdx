...
Elementary has a Slack integration to send alerts about failures in dbt tests, Elementary tests, models runs and
snapshots runs.
...
By default, the alerts that are sent are failed tests, errored models and errored snapshots. You can also monitor source
freshness by running `dbt run-operation upload_source_freshness` right after `dbt source freshness`.
