...
You can share the report via Slack as an html attachement. The slack notification will also include a summary of the test results (see [possible configurations below](/quickstart/share-report-ui#slack-results-summary ---possible-configurations)).
...
You can host the report on AWS or GCP. The slack notification will also include a summary of the test results.
After you [set up a Slack app and token](/integrations/slack#slack-integration-setup) you can run the command:
...
### Slack results summary - possible configurations

#### Filter results summary 

You can filter your results summary by a specific tag, owner or model as can be seen below.
...
By default the test results summary does not include the tests discriptions to avoid reaching Slack's rows limit. 
You can choose to include the test descriptions as part of your summary however, it is reccomonded only if you have a small number of failures.
You can add the descriptions by using the following `include` config:
