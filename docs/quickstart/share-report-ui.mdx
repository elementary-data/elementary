...
After you [setup a Slack app and token](../integrations/slack#slack-integration-setup) you can run the command:

```shell
edr monitor send-report --slack-token <SLACK_TOKEN> --slack-channel-name <CHANNEL_NAME>
```

You can control the name of the report by providing `--slack-file-name <REPORT_NAME>` to the `edr` command.
The report will be sent to your Slack channel under the provided name:
...
You can control the name of the report by providing `--bucket-file-path <REPORT_NAME>` to the `edr` command.
The report will be uploaded to your bucket under the provided name:

- Make sure the report name you provided has a suffix of `.html`.
- If you wish to upload the report into a folder in your bucket, you should add the folder name to the provided report name: `folder/report_name.html`
...
a [Google service account file](https://cloud.google.com/iam/docs/creating-managing-service-account-keys) and
a [GCS bucket](https://cloud.google.com/storage/docs/creating-buckets).
Once you have it, you can send the report to your GCS bucket by running the command:
