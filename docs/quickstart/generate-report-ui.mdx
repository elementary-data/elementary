...
```shell
edr monitor report --select last_invocation
```

To filter by invocation time:

```shell
edr monitor report --select "invocation_time:2022-12-25 10:10:35"
```

To filter by invocation id:

```shell
edr monitor report --select invocation_id:XXXXXXXXXXXXX
```
