...
A custom SQL test is easier to write than a new generic dbt test, but it can't be leveraged across differnt data sets.
...
Additionaly, if you configured a `tested table`, `tag` or `owner`, the test result will be visible under the relevant path. 
