...
- **Incident** - A data issue that starts with an event, but can include several events grouped to an incident. An incident has a start time, status, assigne and end time. 
...
You can create distribution rules to route alers to the relevant people and channels, for faster response. 
