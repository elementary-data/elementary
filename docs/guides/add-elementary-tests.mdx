...
Executes dimension values anomaly detection.
It monitors the frequency of field values (row count for groups based on given columns/expressions) and can be configured using the `dimensions` and `where_expression` keys.
