...
running it will generate a schema_changes_from_baseline test for all sources and models, but it can be customized with
the following arguments:
* `name` - run on a specific source / model
* `include_models` / `include_sources` - whether or not to generate tests for models / sources (by default both are true)
...
Examples:
```shell
# Generate a schema changes from baseline test for all sources and models
...
# Generate a schema changes from baseline test for sources only
dbt run-operation elementary.generate_schema_baseline_test --args '{"include_models": false}'
