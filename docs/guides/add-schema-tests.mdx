...
Snowflake note: snowflake has "synonym" data types, for example tables created with a `VARCHAR` column would be shown
in the information schema as having `TEXT` column. Types are converted internally to deal with this.
...
<Accordion title="elementary.json_schema">
...
<Accordion title="How to auto-generate schema baseline">
...
<Accordion title="How to auto-generate the JSON schema test">
