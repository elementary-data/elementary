...
In order to visualize the data from the [dbt-package](../overview/contributions#guide-on-contributing-to-the-dbt-package) tables, you can use the [CLI](../learn/cli-install) to generate the Elementary report.
...
- If you want to change the schema elementary uses, please see [install & configuration](../getting-started/quickstart-data-monitoring#install-the-dbt-package)
...
Before you can start using Elementary CLI, execute `dbt run` with the [Elementary package](../learn/dbt-package) models, which are **required for Elementary CLI features to work.**
