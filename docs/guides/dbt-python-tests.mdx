---
title: "Beta feature: dbt Python tests"
---

## Python in dbt

In dbt v1.3.0, Python models were introduced in order to solve use cases that you can't solve with SQL.
Using Python allows you to perform analyses using tools available in the open source Python ecosystem, including state-of-the-art packages for data science and statistics.
Before, you would have needed separate infrastructure and orchestration to run Python.

For more information, review the [Python models documentation](https://docs.getdbt.com/docs/building-a-dbt-project/building-models/python-models).

## What about tests?

Following this logic, we'd also like to be able to test our data using those powerful tools and the convenient Python runtime.
Certain problems such as validating JSONs or comparing tables become tremendously easier to solve.
Currently, dbt doesn't provide an out-of-the-box mechanism to run tests using Python, but using dbt's building blocks, Elementary now has the ability of doing so.
...
Let's start with a basic example and proceed gradually to show some of the potential of Python tests.

A test is defined like any other dbt test.
...
Then, we define a macro under `macros/<macro>.sql` that contains the Python code:
...
3. `session`: A `Session` object, either [Snowpark](https://docs.snowflake.com/en/developer-guide/snowpark/reference/python/_autosummary/snowflake.snowpark.html#snowflake.snowpark.Session) or [PySpark](https://spark.apache.org/docs/3.1.1/api/python/reference/api/pyspark.sql.SparkSession.html).
...
In the example above, the test filters out rows that their `ORDER_STATUS` column is not `delivered` and returns the new `DataFrame`.
...
This argument allows us to make our tests generic, meaning that we can simply change the `other_table` field and the compared table will be different without ever touching
the Python code.
  ...
  - name: elementary_test_results
    tests:
      - elementary.python:
          ...
          macro_args:
            schema: "{'type': 'object', 'properties': {'column_name': {'type': 'string'}}}"
            column: test_params
          packages: ['jsonschema']

```

Here we'll be testing that the `column_name` that is provided in the `test_params` in the `elementary_test_results` model is a `string`.
...
In the test's definition, we're passing a `packages` argument which lists additional PyPI Python packages that are required for the code to work.
Here, we've added `jsonschema` to our requirements. Using the `macro_args`, we're specifying the JSON schema that we want to test against, and the column name.
...
At the end, we return the amount of invalid rows. IN this example we're returning an `int` compared to the other in which we returned a `DataFrame`.
...
- `def test(model_df, ref, session)` Python function:
    - Arguments
        ...
        - `session: Session`: A `Session` object, either [Snowpark](https://docs.snowflake.com/en/developer-guide/snowpark/reference/python/_autosummary/snowflake.snowpark.html#snowflake.snowpark.Session) or [PySpark](https://spark.apache.org/docs/3.1.1/api/python/reference/api/pyspark.sql.SparkSession.html).
    - Returns
        - `DataFrame`: Either a Spark or a [Pandas](https://pandas.pydata.org/) `DataFrame`.
          The test's failed results count will be the amount of rows in the `DataFrame`.
...
Given that Elementary's Python tests base on Python models' building blocks, the requirements are the same and can be found [here](https://docs.getdbt.com/docs/building-a-dbt-project/building-models/python-models#specific-data-warehouses).

## Beta feature

This is a beta feature of Elementary.
Also, running Python within the warehouse is a new feature in dbt and therefore there might be some bumps along the road.
