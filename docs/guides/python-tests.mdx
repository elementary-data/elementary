...
Following this logic, we'd also like to be able to test our data using those powerful tools and the convenient Python
runtime.
Certain problems such as validating JSONs or comparing tables become tremendously easier to solve.
Currently, dbt doesn't provide an out-of-the-box mechanism to run tests using Python, but using dbt's building blocks,
Elementary now has the ability of doing so.
...
Here we'll be testing that the `column_name` that is provided in the `test_params` in the `elementary_test_results`
model is a `string`.
...
Running Python within the warehouse is a new feature in dbt and therefore there might be some bumps along the road.
