...
1. [Test param](#test-arguments) - Will apply to the test in which it is configured.
...
Configure Slack integration by creating an app for Elementary and adding the `Token/Webhook` to `config.yml`. Refer to [this guide](../integrations/slack) for detailed instructions.
