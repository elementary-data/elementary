...
Filter the tested data using a valid sql expression. Example usage: `exlude_regexp: "user_name = 'test'"`

</Accordion>
<Accordion title="seasonality">
`seasonality`: [string]

Compare data of a specific point in time to not-adjacent previous data points.
Currently only support `seasonality: day_of_week` with `time_bucket` of 1 day.
day_of_week means each daily bucket would be compared to the daily bucket of 1 week prior to it, 2 weeks prior, 3 weeks prior, ...
