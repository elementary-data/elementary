...
Elementary has a UI for visualization and exploration of data from the [dbt-package](../project-overview/contributions#guide-on-contributing-to-the-dbt-package) tables, which includes dbt test results, Elementary anomaly detection results, dbt artifacts, etc.

After installing and configuring the [CLI](../understand-elementary/cli-install) and [dbt package](../quickstart#install-the-dbt-package), execute the command:
...
The `Test results` tab shows the latest result for each dbt or Elementary test that was executed in the last 7 days (Can be changed by passing `--days-back` param to the [CLI](../understand-elementary/cli-advanced)).
...
The `Test runs` tab shows the last 30 runs (can be changed by passing `--executions-limit` param to the [CLI](../understand-elementary/cli-advanced)) for each dbt or Elementary test that was executed in the last 7 days (can be changed by passing `--days-back` param to the [CLI](../understand-elementary/cli-advanced)).
...
<img
  src="https://res.cloudinary.com/mintlify/image/upload/v1659304885/elementary/Test-Runs-video-demo_ubxssd.gif"
  alt="Test runs demo"
/>

## Exporting the report

The data tests report UI can be sent via Slack, Amazon S3, or Google Cloud Storage when you run `edr monitor send-report`.

Refer to [this guide](../quickstart/send-report-ui) for detailed instructions.
