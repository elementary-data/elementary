...
4. Click **Save**.  
5. Step 5 involves OAuth authentication, a protocol used to authenticate between the customer's service (Azure DevOps) and Elementary Cloud. During this process, a temporary token is issued, which can be used to make API calls. Along with the temporary token, a refresh token is also provided. The refresh token is used when the customer's service (Azure DevOps) indicates that the temporary token has expired. For Microsoft services, OAuth is managed by Microsoft Entra ID (formerly known as Active Directory).
