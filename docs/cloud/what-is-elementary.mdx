...
Elementary is native to the modern data stack. You connect your data warehouse to get started - Elementary reads warehouse metadata and runs cloud tests to monitor freshness, volume, schema, and custom rules, with no dbt project required. If you use dbt, the open-source Elementary dbt package adds automatic collection of your dbt test results and artifacts, and enables Elementary's anomaly detection tests in your pipeline. The same foundation powers both the self-hosted CLI and the fully managed Cloud Platform, which builds on it to provide wider visibility, automations, scale, and advanced reports.
...
Learn more about Elementary Cloud’s features and integrations [here](https://docs.elementary-data.com/cloud/introduction), and [talk to the team](https://meetings-eu1.hubspot.com/joost-boonzajer-flaes/intro-call-docs) to get started.
