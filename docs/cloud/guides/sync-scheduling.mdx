...
In order to configure the sync webhook, open the [_Environments_](https://app.elementary-data.com/settings/environments) tab in Elementary Cloud and click on your environment's _Schedule Settings_. The next step depends on whether you're using dbt Cloud or dbt Core.

#### dbt Cloud

In the _Schedule Settings_, you're provided with a webhook URL and you'll need to provide a secret token that will be returned from dbt Cloud at the end of the next step.
