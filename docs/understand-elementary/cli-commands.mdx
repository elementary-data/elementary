...
```shell
-q, --dbt-quoting [all|none|quoting config json]

# If a quoting config json is supplied, it should be of the following format:
{"database": true/false/null, "schema": true/false/null, "identifier": true/false/null}
# (true - use quoting, false - don't use quoting, none - use dbt's default according to the adapter being used)
```
