...
We store _artifacts_, which is information about dbt resources such as models, tests, etc.
You can view them as models called `dbt_models`, `dbt_tests`, `dbt_exposures`, and so on.

We internally use these models in order to gather the latest information on your dbt resources.
...
For our alerts to work properly, we'd want to make sure that the artifact models have been built according to the latest changes that were made.
