...
After you successfully [installed Elementary](../learn/cli-install#install-and-configure), [configured a connection profile](../learn/cli-install#cli-configuration) and provided the permissions required to access the [query history](), it's time to create your data lineage graph using the lineage CLI.

```shell
edr lineage
```

<img src="../img/elementary_demo.gif" alt="Elementary Demo" />
...
The lineage is created from the data warehouse [query history](./query-history) and access history, and the timeframe filters are used to define which queries to include in the graph.
