...
Depending on your project custom schemas rules (see [dbt custom schema docs](https://docs.getdbt.com/docs/build/custom-schemas)), the schema will be named `elementary` or `<target_schema>_elementary` (very often `<target_schema> = analytics`).

Make sure your user (or the orchestrator user) has permissions to create schemas.
If not, ask your admin to create the elementary schema and grant permissions to the relevant users / groups.
