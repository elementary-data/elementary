...
vars:
  ...
  show_sample_rows_tags: ['show_sample_rows']        # Tags that allow samples (default: ['show_sample_rows'])
```
...
Both features can be enabled at the same time. **PII always takes precedence** -- if a model or column has a PII tag, samples remain hidden even if `show_sample_rows` is also present.
...
| Model tags | Column tags | Samples shown? |
| --- | --- | --- |
| `show_sample_rows` | -- | Yes |
| `pii` | -- | No |
| `pii` + `show_sample_rows` | -- | No (PII wins) |
| `show_sample_rows` | `pii` on test column | No (PII wins) |
| `show_sample_rows` | `show_sample_rows` on test column | Yes |
| none | none | No (hidden by default) |
