...
1. Measure the selected column metrics per time bucket and store them in Elementary's `data_monitoring_metrics` table. This history is kept separately from anomaly tests' metrics, so running both tests on the same column scans the table for each
...
_Required configuration: `columns`, `metrics`, `timestamp_column`, `min_bucket_age`. `timestamp_column` can be set on the test or inherited from the model's Elementary config._
...
| `timestamp_column`   | Yes      | -              | Column that buckets the data into periods. Set it on the test or in the model's Elementary config |
...
| `backfill_days`      | No       | `days_back`    | Days re-measured on each run, for incremental models and sources. Must be at least the derived minimum window |
...
`days_back` bounds the observation window. The drift example above protects daily buckets roughly 28 to 90 days old, not all historical data, and corrections outside that window are not detected. Incremental models and sources also use `backfill_days` to control re-measurement; it defaults to `days_back`. A `backfill_days` below the minimum window derived from `min_bucket_age` raises an error, because those buckets would stop being re-measured before they become eligible to check. A value between that minimum and `days_back` is allowed, and reduces coverage to the buckets actually scanned on that run. Setting the `force_metrics_backfill` var re-measures the full `days_back` window on every run regardless of `backfill_days`.
