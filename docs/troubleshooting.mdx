...
- Refer to the [dbt package installation guide](/quickstart-data-monitoring#install-the-dbt-package), and validate that your version in packages.yml is the one mentioned there. If not, upgrade and run `dbt deps`. Make sure to execute `dbt run --select elementary` for the package tables to be created.

- **Check if the table `elementary_test_results` exists and has data**
- If the table does not exist - refer to the [dbt package installation guide](/quickstart-data-monitoring#install-the-dbt-package). Make sure to execute `dbt run --select elementary` for the package tables to be created.
...
3. Some query types used are [not supported](/query-history#unsupported-query-types), you can notify us on a [GitHub issue](https://github.com/elementary-data/elementary-lineage/issues) and we will include these in our next release
