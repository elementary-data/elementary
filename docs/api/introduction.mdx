...
| Asset health scores     | `GET /{env_id}/asset-health-scores`               | Current health score per asset                  |

Every list endpoint supports a full scan and keyset pagination. Assets,
columns, and tests also expose incremental feeds — see
[Incremental sync](/api/incremental-sync). Latest test executions and asset
health scores are full snapshots; per-test history is a time window.
...
**[`GET /asset-health-scores`](/api/reference/asset-health-scores/list-asset-health-scores)**
returns the current data health score of every monitored asset, the same number
shown in the Elementary data health dashboard when looking at a single asset.
Use it to feed scorecards or nightly quality reports.

Each item has a `total` score and one entry per quality `dimension`
(`completeness`, `uniqueness`, `freshness`, `validity`, `accuracy`,
`consistency`):

```json
{
  "asset_id": "model.analytics.orders",
  "total": { "score": 0.91, "test_count": 12, "bucket_at": "2026-09-14T08:00:00" },
  "dimensions": [
    { "dimension": "freshness", "score": 0.99, "test_count": 3, "bucket_at": "2026-09-14T08:00:00" }
  ]
}
```

- Scores are in the range `0`–`1`. `total` is the weighted combination of the
  dimensions using the environment's configured weights.
- Scores are computed from test results in the **last 7 days**, in hourly
  buckets; `bucket_at` is the start of the most recent bucket with results.
  Dimensions with no results in that window are omitted, and assets with no
  results at all are not returned.
- Full snapshot, replaced in place — re-read it; there is no incremental feed.
  The only filter is `asset_ids`. Join to
  [assets](/api/reference/assets/list-assets) on `asset_id`.
- `dimension` is an extensible enum (see below).
...
`definition_type`, `status`, `dimension`), but that set
