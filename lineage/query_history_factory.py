...
class QueryHistoryFactory(object):
    ...
    def create_query_history(self, credentials: Any, profile_data: dict) -> QueryHistory:
        ...
        if credentials_type == 'snowflake':
            ...
            return SnowflakeQueryHistory(snowflake_con, self.database_name, self.schema_name, self._export_query_history,
                                         self._full_table_names, profile_data.get('query_history_source'))
        elif credentials_type == 'bigquery':
            bigquery_client = get_bigquery_client(credentials)
            return BigQueryQueryHistory(bigquery_client, self.database_name, self.schema_name, self._export_query_history,
