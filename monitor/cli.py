...
def send_report(
    ...
    try:
        if not config.slack_token or not config.slack_channel_name:
            logger.error('You must provide a Slack token and a channel in order to send a report.')
            return 1

        data_monitoring = DataMonitoring(config=config, force_update_dbt_package=update_dbt_package)
        command_succeeded = False
        generated_report_successfully, elementary_html_path = data_monitoring.generate_report(
            days_back=days_back, test_runs_amount=executions_limit,
...
def upload_report(
    ...
    config = Config(config_dir, profiles_dir, profile_target, update_bucket_website=update_bucket_website,
    ...
    anonymous_tracking = AnonymousTracking(config)
    track_cli_start(anonymous_tracking, 'monitor-upload-report', get_cli_properties(), ctx.command.name)
    try:
        if not config.has_upload_platform:
            logger.error('You must provide a platform to upload the report to (AWS / GCS).')
            return 1
        ...
        generated_report_successfully, elementary_html_path = data_monitoring.generate_report(
            ...
            disable_passed_test_metrics=disable_passed_test_metrics, should_open_browser=False)
        if generated_report_successfully and elementary_html_path:
            command_succeeded = data_monitoring.upload_report(elementary_html_path)
        return 0 if command_succeeded else 1

    except Exception as exc:
        track_cli_exception(anonymous_tracking, 'monitor-upload-report', exc, ctx.command.name)
        raise
