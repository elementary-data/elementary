...
from observability.alerts import SchemaChangeAlert, Alert
...
class DataMonitoring(object):
    ...
    DBT_PROJECT_PACKAGES_DIR = 'dbt_packages'
    ...
    def __init__(self, config: 'Config', db_connection: Any) -> None:
        self.config = config
        self.dbt_project_path = os.path.join(FILE_DIR, self.DBT_PROJECT_DIR)
    ...
    def _dbt_package_exists(self) -> bool:
        dbt_package_path = os.path.join(self.dbt_project_path, self.DBT_PROJECT_PACKAGES_DIR, self.DBT_PACKAGE_NAME)
    ...
    def _update_configuration(self) -> bool:
        target_csv_dir = os.path.join(self.dbt_project_path, self.DBT_SEEDS_DIR)
