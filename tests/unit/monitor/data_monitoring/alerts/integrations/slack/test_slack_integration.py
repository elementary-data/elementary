...
def test_get_alert_template_workflow_param():
    ...
    assert workflow_integration._get_alert_template(alert=alerts_data.dbt_test).text == json.dumps(
        alerts_data.dbt_test.data, sort_keys=True
    )
...
def test_get_integration_params_different_slack_clients_logic():
    ...
    assert json.dumps(webhook_client_integration._get_integration_params(alert=alerts_data.dbt_test)) == json.dumps({})

    web_client_integration = get_slack_integration_mock(slack_token="mock", slack_channel_name="mock")
    assert json.dumps(web_client_integration._get_integration_params(alert=alerts_data.dbt_test)) == json.dumps(
        {"channel": "mock"}
    )
